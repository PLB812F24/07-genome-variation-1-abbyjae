...
geom_histogram(binwidth=4) #the type of plot we want to make
```
...
```{r}
ggplot(penguins, aes(x = flipper_length_mm)) + #information about the dataset
  geom_histogram(binwidth=4, color = "magenta", fill="pink") + #make the figure
...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species)) +
  geom_point() +
  theme_minimal() +
  scale_colour_manual(values = c("magenta", "blue", "darkviolet")) + #change the color
...
```{r}
ggplot(penguins, aes(x = body_mass_g)) + #information about the dataset
  geom_histogram(binwidth=150, color = "plum", fill="pink1") + #make the figure
  labs(x = "Flipper Length (mm)", y = "Frequency") + #add labels
theme_minimal() #get rid of the default gray background
```
...
ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = species)) +
  geom_point() +
  theme_minimal() +
  scale_colour_manual(values = c("hotpink", "blue", "magenta")) + #change the color
geom_point(size = 2) #change the size 
```
...
```
```{r}
ggplot(penguins, aes(x = body_mass_g, y = sex, color = species)) +
  geom_point() +
  theme_minimal() +
  scale_colour_manual(values = c("hotpink", "blue", "magenta")) + #change the color
geom_point(size = 2) #change the size 
...
scale_fill_manual(values=c("magenta", "blue", "pink")) #the alpha affects transparency
